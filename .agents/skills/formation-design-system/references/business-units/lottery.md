---
source: tokens-pkg
note: Auto-derivable from @fanduel/formation-tokens/build/json/lottery.json
---
# Lottery Theme Extension

> **AI TOOLS: STOP. DO NOT USE THIS FILE.**
> This file is a placeholder draft — it contains NO real token values. All token references like "Lottery brand color" are fictional.
> The Lottery theme is **not yet implemented** in `@fanduel/formation-tokens`.
> For real token values, use `references/core/colors.md` and `AGENTS.md` only.
> The Lottery theme is not yet implemented in `@fanduel/formation-tokens`.
...
Apply theme via HTML attribute (same as all Formation themes — no ThemeProvider component):

```html
<body data-theme="lottery">
  <!-- Lottery app content -->
</body>
```

> Note: Lottery tokens are not yet in `@fanduel/formation-tokens`. Use `data-theme="fanduel"` and Formation base tokens until the theme is released.
